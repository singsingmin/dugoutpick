...
아래 **이 phase에 필요한 범위만** 선별해 읽어라(콜드 세션이므로 전체 문서를 통째로 읽으면 토큰 낭비 — 가능하면 라인 범위까지 좁혀라):

- {이 phase가 실제로 의존하는 문서·섹션만 나열 — 예: `docs/spec.md` 트랙레코드 계약 부분, `docs/data-schema.md` recap-history 구조. 전체 docs/ 나열 금지}
- `/tasks/{id}-{name}/docs-diff.md` (이번 task의 문서 변경 기록 — 변경점만 빠르게 파악용)

그리고 이전 phase 작업물 중 **이 phase가 직접 import/수정/검증하는 파일만** 확인하라(라인 범위 명시 권장):

- {이전 phase에서 생성/수정된 파일 중 직접 관련된 것만 나열}
...
3. **사전 준비는 최소 범위로**: 이 phase가 실제 의존하는 문서·섹션·파일만 명시하고, 가능하면 라인 범위까지 좁혀라. 각 phase는 콜드 session이라 docs/ 전체를 매번 재독하면 토큰이 phase 수만큼 곱으로 낭비된다. "전체 아키텍처 이해" 같은 광범위 지시 대신 "이 작업에 필요한 X의 Y 부분"으로 핀포인트.
