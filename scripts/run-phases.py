...
중요한 규칙:
1. 작업 전에 이 phase의 "사전 준비"에 **명시된 문서·코드 범위만** 선별해서 읽으세요. 라인 범위가 지정돼 있으면 그 범위만 읽고, 전체 docs/ 를 통째로 훑지 마세요(콜드 세션 토큰 낭비). 명시 범위로 부족할 때만 추가로 확장하세요.
2. 이전 phase에서 작성된, 이 작업과 직접 관련된 코드만 읽고 기존 코드와의 일관성을 유지하세요.
