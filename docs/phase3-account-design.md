...
### 넛지 상호작용 (결정 2026-07-04, ✅ 구현 = `components/ProtectNudge.tsx`, LockerRoom 상단)
**패턴 = 닫기 가능한 인라인 배너 + CTA 버튼.** 노출 = 미연결 + (첫 스킨 구매[ledger reason=skin_purchase] or 7일 연속 출석) + 미닫힘. CTA="구글 계정 연결하기"→Settings. ✕=dismiss(로컬 플래그). 연결되면 자동 사라짐. 자동으로 계정화면 이동은 **금지**(방금 스킨 산 유저를 로그인 화면으로 휙 던지면 흐름 끊김·강매/페이월 느낌 → 전환율 역효과).
- 트리거 직후 배너 표시: 예) "🔒 스킨 샀네요! 지금 보호 안 하면 앱 삭제 시 사라져요 **[지금 보호하기]** ✕"
- **[지금 보호하기]** 탭 → 그때 `AccountProtect`로 이동(원할 때만·한 탭). **✕** → dismiss + 재노출 안 함(기기 로컬 플래그).
- "알림만"은 행동 유도 약하고 "강제 이동"은 거슬림 → 배너(비침습)+버튼(저마찰)이 둘의 장점만.
