...
> **야구공 재화의 서버/DB 이후 핵심 사용처 = "꿀잼 예측 리그"**(아래 Phase 4). 스킨은 소비처, 예측 리그는 참여·보상 루프의 중심. Phase 3(계정/DB)~Phase 4-부속(추천코드)까지 구현 완료, 남은 유일한 블로커는 [F섹션](#f-🚀-출시-준비-android)의 Supabase 마이그레이션 실행.
>
...
### 현실 조언 (기능은 비용)
- Phase 1~4(스킨·로컬 MVP·계정/DB·예측 리그) + Phase 4-부속(추천코드)까지 **구현 완료**. 남은 유일한 서버측 블로커는 Supabase 마이그레이션 실행([F섹션](#f-🚀-출시-준비-android)).
