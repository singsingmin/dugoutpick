...
// '지금 볼 각' 라이브 흥미도/라벨 raw core — liveHeatCore.mjs(순수 모듈)에서 import.
// Worker(cf-worker/src/index.ts)에 동일 로직 복제. momentum/smooth는 앱 전용(ADR-021).
// 단위 테스트: data-pipeline/test/liveheat.test.mjs
  ...
  const games = rawGames.map(g => {
    ...
    if (status === 'LIVE') {
      ...
      live = {
        ...
        heat: rawLiveHeat(inning, half, diff, total),
