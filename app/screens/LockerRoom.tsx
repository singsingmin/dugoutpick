...
// 배경만 보기(peek): 스킨 장착 시 '크게 보기'로 UI를 페이드아웃 → 화면 탭하면 복귀.
import { useCallback, useRef, useState } from 'react';
import { View, ScrollView, Image, Pressable, Animated, StyleSheet } from 'react-native';
...
// 복귀 방법 힌트는 앱 실행당 1회만(반복 노출 방지). 세션 수준이라 저장소 불필요.
let peekHintShown = false;
...
export default function LockerRoom() {
  ...
  const enterPeek = useCallback(() => {
    ...
    if (!peekHintShown) { peekHintShown = true; showToast('화면을 탭하면 돌아와요'); }
  }, [chromeOpacity, showToast]);

  const exitPeek = useCallback(() => {
    Animated.timing(chromeOpacity, { toValue: 1, duration: 200, useNativeDriver: true })
      .start(({ finished }) => { if (finished) setPeek(false); });
  }, [chromeOpacity]);
  ...
  return (
    <View style={styles.root}>
      ...
      <Animated.View style={[styles.chrome, { opacity: chromeOpacity }]} pointerEvents={peek ? 'none' : 'auto'}>
        <SafeAreaView style={styles.safe} edges={['top']}>
          ...
          <ScrollView contentContainerStyle={styles.content}>
            ...
            {equippedLabel && (
              <Pressable
                style={styles.equippedPill}
                onPress={enterPeek}
                hitSlop={8}
                accessibilityRole="button"
                accessibilityLabel={`배경만 크게 보기, 현재 ${equippedLabel}`}
              >
                <PixelText variant="caption" color="#fff">적용 중 · {equippedLabel} · 크게 보기 ›</PixelText>
              </Pressable>
            )}
      ...
      {/* 배경만 보기 중 — 화면 아무데나 탭하면 UI 복귀 */}
      {peek && (
        <Pressable
          style={StyleSheet.absoluteFill}
          onPress={exitPeek}
          accessibilityRole="button"
          accessibilityLabel="UI 다시 보기"
        />
      )}
